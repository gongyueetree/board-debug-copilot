...
### 1.5.1 `aout.setCyclic` 的真实签名 —— 代码已自适应

**这一条不再需要你改代码**，但仍然要记录结果。

两份独立实现给出了不同写法：我们原本是 `setCyclic(True)`，另一份
（T28 交付包的 `libm2k_backend.py`）是 `setCyclic(idx, True)`。
**两份都没在真机上跑过**，所以不是谁对谁错的问题 —— 现在代码两种都试，
生效的那个签名会出现在响应的 `cyclicSignature` 字段和 `/diagnostics` 的 notes 里。

**为什么要紧**：不设循环的话，AWG 把缓冲播完就停，输出变成**一个脉冲**而不是
连续波形。示波器上看到的是一条直线，很容易被当成「没有输出」而去查接线。

| 检查 | 怎么做 | 记录 |
| --- | --- | --- |
| 哪个签名生效 | 配一次 AWG，看返回体的 `cyclicSignature` | ☐ ______________ |
| 两个都失败？ | notes 里会是 `全部签名都失败` 并列出两个报错 | ☐ |
| 输出确实是连续波形 | 示波器上连续观察十几秒 | ☐ |
| 若两个都失败时的输出 | 是连续波形还是单个脉冲 | ☐ |

把生效的签名填在上面 —— 确认之后可以把另一个分支删掉，减少一次无谓的异常。

### 1.5.1b AWG 缓冲的 4 对齐要求

另一份实现里写着「libm2k 循环 buffer 的样点数必须按 4 对齐」。同样没有真机
佐证，但我们采纳了它，因为**零代价**：多周期搜索总能找到既 4 对齐、
频率误差又为 0 的方案（1MHz 从 k=1/N=75 换成 k=4/N=300）。

| 检查 | 记录 |
| --- | --- |
| 4 对齐的缓冲能正常 push | ☐ |
| 故意传非 4 倍数会怎样（改 `AWG_BUFFER_ALIGN=1` 试 1MHz） | ☐ ______________ |

第二行是关键：如果非对齐也能正常输出，说明这个约束不存在，可以去掉；
如果 push 失败或波形出错，说明约束是真的，记进本文。
...
| 只使能一个通道时返回几个通道 | 另一份实现说「禁用的通道不占位」 | | ☐ |

最后一行我们规避掉了（连接时两个通道都使能），但值得确认 —— 若真是「不占位」，
将来做单通道模式时会踩到。

### 1.5.4 换量程之后的首个 buffer

另一份实现里丢弃换量程/校准之后的第一个 buffer，理由是那一帧有跳变。
我们采纳了（`_discard_next`）。

**为什么要紧**：不丢的话每次调量程都会看到一个假尖峰，而它看起来
**完全像是被测电路上的毛刺** —— 这种「像真的一样的假象」最费排查时间。

| 检查 | 记录 |
| --- | --- |
| 换量程后第一帧确实有跳变（把 `_discard_next` 改成不丢再看一次） | ☐ |
| 丢掉之后波形干净 | ☐ |
...
1. **§1.5 各条重点项全部确认**（`setCyclic` 签名、采样率表、`getSamples` 形状与量纲）
