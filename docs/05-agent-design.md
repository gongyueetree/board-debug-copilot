...
## 17. 真实 provider 接入后的实证记录

Mock 基线全绿（12/12）时，以下六个问题一个都看不见。记录在此，因为它们全都不是
"模型不够聪明"，而是**契约、边界或防线本身的缺陷**。

| # | 现象 | 根因 | 修法 |
|---|---|---|---|
| 1 | 每次结构化调用都降级，报「JSON 括号不平衡」 | `maxOutputTokens=4096`，而 2.5 系列是思考模型，推理与输出**共用预算**，JSON 没写完 | 上限提到 16384；启用原生 JSON 模式；`finishReason=MAX_TOKENS` 显式抛错 |
| 2 | `design_review` 的 LLM 部分**一直**静默降级 | 校验 schema 要求 `origin`，但那是系统赋值字段，模型不可能也不该填 | 模型面对的 schema `omit({origin})` |
| 3 | 流式"成功"但一个 token 都没有 | Gemini 的 SSE 用 **CRLF** 分隔，按 `\n\n` 切永远切不出事件 | 三个 provider 共用 `sse.ts`，按 `/\r?\n\r?\n/` 切 |
| 4 | 偶发 500，ZodError | grounding 把 `recommendations` 过滤空，违反 `min(1)` | 数组被清空说明整体不可信，退回规则结论而非产出半个对象 |
| 5 | AI 的新发现全部消失 | `slice(0,12)` 前按 **origin** 排序，RULE_ENGINE 占满配额 | 截断前按**严重度**排序 |
| 6 | `normal` 场景判 CRITICAL / 给故障 code | 模型把「设计缺陷有多严重」和「本次测量有多严重」混为一谈 | L2 规则是本次测量的权威：没检出故障时 severity 封顶 WARNING、primaryCode 清空 |
| 7 | 视觉分析对上传的照片永远返回 0 条 | 判的是 `objectKey.startsWith('data:')`，但上传照片的 objectKey 是对象存储路径，多模态分支根本不可达；schema 失败还被静默吞掉 | 经 StorageService 回读图像；解析失败必须记日志 |
| 8 | 视觉 schema 每次都失败 | 校验 schema 要求 `id`，那是数据库赋值字段 —— 与 #2 的 `origin` 同一类 | 模型面对的 schema 单独定义，落库后回读真实 id |
| 9 | 诊断报 confidence 0.99 | 根因尚未被实测证伪就声称接近确定 | 封顶 0.95：第一条建议就是「能证伪当前根因」的测量，还要靠它验证就不是 99% |

**同一类问题反复出现，值得单独记下：**

- **系统赋值字段混进模型面对的 schema**（#2 的 `origin`、#8 的 `id`）——
  模型不可能也不该填，留在校验 schema 里只会让每次调用静默失败。
  现在统一做法：模型面对的 schema 与响应 schema **分开定义**。
- **#6 出现了两次**（先 primaryCode，修完又 severity），是同一根因的两个面。
  凡是下游要据以分支的字段，都必须由确定性层约束，不能只靠 prompt 说明。
- **失败必须留痕**（#7）。静默 catch 让「0 条结果」无从追查，
  加上日志后下一个问题（#8）立刻就浮出来了。

**评测断言结构而非措辞的价值在 #6 上体现得最直接**：`gain_error` 的根因文本里有
「可排除**削顶**」，字符串匹配会把正确的排除推理判成误诊。`primaryCode` 就是为此引入的。

### 稳定性

修复后连跑**六轮**，11/11 全绿（1 条需本地 Bridge 跳过）。

模型输出有非确定性，**单次通过不算通过**：#6 修完第一轮就绿，第二轮换个面又挂；
#9 也是第二轮才暴露。验收至少跑三轮。
