...
export class AssemblyInspectionService {
  ...
  async inspect(photoId: string): Promise<AssemblyInspectionResult> {
    ...
    const pcb = await this.prisma.projectFile.findFirst({
      where: { projectId: photo.projectId, kind: 'PCB' },
      orderBy: { createdAt: 'desc' },
    })
    if (!pcb) throw new NotFoundException('当前项目没有已解析的 .kicad_pcb 文件，请先上传并解析 KiCad 工程')

    const [photoBuf, pcbBuf] = await Promise.all([
      this.storage.get(photo.objectKey),
      this.storage.get(pcb.objectKey),
    ])
    if (!photoBuf) throw new NotFoundException('PCB 实物照片文件不存在')
    if (!pcbBuf) throw new NotFoundException('KiCad PCB 文件不存在')

    const map = parsePcbAssembly(pcbBuf.toString('utf8'))
    ...
    const prompt = [
      ...
      '',
      '下面的 KiCad 数据来自 .kicad_pcb。每一行代表一个 footprint；padCount/padNumbers 明确了哪些焊盘属于同一个器件。',
      'x/y 是相对整板元器件范围归一化坐标。照片可能有透视、旋转或镜像；先用板框、安装孔、连接器、丝印和器件群建立对应关系。',
      '',
      '判定规则：',
