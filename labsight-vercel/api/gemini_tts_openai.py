...
app = FastAPI(title="LabSight Gemini TTS OpenAI Bridge", version="0.1.4")
...
def health():
    now = time.time()
    last_at = float(LAST_REQUEST.get("at", 0) or 0)
    return {
        "ok": True,
        ...
        "last_request": ({
            "age_seconds": round(max(0, now - last_at), 3),
            "user_agent": LAST_REQUEST.get("user_agent"),
            "input_chars": LAST_REQUEST.get("input_chars"),
            "status": LAST_REQUEST.get("status"),
            "latency_ms": LAST_REQUEST.get("latency_ms"),
            "bytes": LAST_REQUEST.get("bytes"),
    ...
    try:
        ...
        for attempt in range(2):
            ...
            if response.status_code == 429:
                ...
                raise HTTPException(status_code=429, detail={"code": "gemini_tts_quota_exhausted", "message": "Gemini TTS 配额已耗尽", "provider_message": message, **meta})
            if 400 <= response.status_code < 500:
                message, meta = _parse_google_error(response)
                raise HTTPException(status_code=response.status_code, detail={"code": "gemini_tts_request_rejected", "message": message, **meta})
            ...
            try:
                ...
                return Response(
                    ...
                    headers={"X-Audio-Format": "pcm_s16le", "X-Sample-Rate": "24000", "X-Channels": "1", "Cache-Control": "no-store"},
                )
        ...
        LAST_REQUEST.update({"status": "error", "latency_ms": round((time.perf_counter() - started) * 1000)})
        raise
