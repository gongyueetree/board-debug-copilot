...
app = FastAPI(title="LabSight Agora OpenAI-Compatible LLM Gateway", version="0.8.0")

SYSTEM_PROMPT = (
    "你是 LabSight 的实时语音调试助手，面向电子研发工程师。"
    "始终使用简体中文，回答短、快、自然，优先 2~5 句话：先给结论，再给下一步。"
    "器件型号、网络名、引脚名、协议名和单位保持原样。"
    "当前 EVT0.8 语音通道还没有自动注入摄像头画面；如果问题明确依赖当前 PCB 或仪器画面，"
    "请直接说明需要用户点击页面中的“分析当前画面”或“PCB Deep Vision”，不要假装看到了画面。"
)
...
def _check_auth(authorization: str | None, x_api_key: str | None) -> None:
    ...
    if supplied != expected:
        raise HTTPException(status_code=401, detail="Invalid LabSight Agora LLM gateway credential")
...
def _openai_stream(messages: list[dict[str, str]]) -> Iterable[bytes]:
    ...
    if not key:
        raise HTTPException(status_code=503, detail="Agora Voice 选择 OpenAI，但未配置 OPENAI_API_KEY")
    model = os.getenv("AGORA_OPENAI_MODEL", "gpt-4o-mini")
    payload = {
        "model": model,
        "messages": messages,
        "stream": True,
...
def _gemini_stream(messages: list[dict[str, str]]) -> Iterable[bytes]:
    ...
    if not key:
        raise HTTPException(status_code=503, detail="Agora Voice 选择 Gemini，但未配置 GEMINI_API_KEY")
    model = os.getenv("AGORA_GEMINI_MODEL", "gemini-2.5-flash")
    system_instruction, contents = _gemini_messages(messages)
    payload: dict[str, Any] = {
        "contents": contents,
        "generationConfig": {
            "temperature": float(os.getenv("AGORA_LLM_TEMPERATURE", "0.35")),
            "maxOutputTokens": int(os.getenv("AGORA_LLM_MAX_TOKENS", "220")),
        },
    }
...
async def agora_chat(
    ...
    if body.get("stream") is False:
        raise HTTPException(status_code=400, detail="Agora Chat Completions requires stream=true")
