...
async def private_network_headers(request, call_next):
    ...
    # Helps Chromium Private Network Access preflights when LabSight is hosted on HTTPS
    # and the bridge runs on localhost.
...
def decode_loop(url: str, redacted: str):
    ...
    while not worker_stop.is_set():
        ...
        try:
            ...
            # tcp is more stable on typical office Wi-Fi; timeout values are microseconds.
            container = av.open(
                url,
                options={
                    "rtsp_transport": "tcp",
...
def configure(req: ConfigureRequest):
    ...
    start_worker(req)
    # Return immediately; the browser polls frame.jpg while RTSP is starting.
    return {
        "ok": True,
        "camera_ip": req.camera_ip,
        "rtsp": redacted_url(req),
        "width": state.width,
        "height": state.height,
    }
